    ...
    <>
      <section className="flex items-start gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {stories.map((story) => (
          <div
            key={story.id}
            className="shrink-0 w-36 sm:w-44 lg:w-60 h-28 sm:h-28 lg:h-24 border border-neutral-300 p-2 cursor-pointer overflow-hidden"
            onClick={() => openModal(story)}
          >
            <p className="text-xs lg:text-sm leading-snug">{story.headline}</p>
        ...
        <div className="modal-box rounded-none">
          {selectedStory && (
            <>
              ...
              <p className="mt-3 text-sm font-light text-neutral-600 leading-relaxed">
                      ...
                      {(selectedStory.related_articles_urls as string[]).map(
                        (url, i) => (
                          <Link
                            ...
                            className="text-neutral-600 text-xs underline hover:underline"
                          >
                            Source {i + 1}
                          </Link>
                        ),
